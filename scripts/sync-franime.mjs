...
// --- Fonctions modifiées pour gérer le chargement dynamique ---

async function openPage(page, url, waitForContent = true) {
  ...
  if (waitForContent) {
    try {
      // Attendre que le texte "Chargement des animés" disparaisse
      await page.waitForFunction(
        () => {
          const text = document.body.innerText || '';
          return !text.includes('Chargement des animés');
        },
        { timeout: 30000 }
      );
      console.log('Chargement terminé');
    } catch (error) {
      console.warn('Le chargement a peut-être échoué, tentative de récupération...');
    }
  }

  // Attendre qu'au moins une carte d'anime soit présente
  try {
    await page.waitForSelector('a[href*="/anime/"]', { timeout: 15000 });
  } catch (error) {
    console.warn(`Aucun lien d'anime trouvé sur ${url}, tentative de rechargement...`);
    await page.reload({ waitUntil: 'domcontentloaded' });
    // Ré-attendre le chargement
    try {
      await page.waitForFunction(
        () => {
          const text = document.body.innerText || '';
...
async function collectCatalog(page) {
  ...
  while (true) {
    ...
    try {
      // On passe waitForContent=true pour attendre la fin du chargement
      await openPage(page, catalogPage, true);
      ...
      // Si aucun lien, tenter de cliquer sur "Charger plus"
      if (links.length === 0) {
        const loadMore = page.locator('button:has-text("Charger plus")');
...
async function detectEpisodeNumbers(
  ...
  await openPage(page, seasonUrl, true);
  await dismissDialogs(page);
...
async function extractSendvid(
  ...
  try {
    ...
    await openPage(page, episodeUrl, true);
    await dismissDialogs(page);
...
async function processAnime(page, anime) {
  ...
  await openPage(page, anime.url, true);
  await dismissDialogs(page);
  await clickLoadEpisodes(page);
